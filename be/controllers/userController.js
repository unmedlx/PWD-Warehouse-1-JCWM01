...
module.exports = {
  ...
  register: (req, res) => {
    ...
    db.query(checkEmailQuery, (err, results) => {
      if (results) {
        res.send({message: "This Email Already Registered", message1:"Try Again With Different Email", success: false});
        return; 
      }else{
        // Hash Pass
            ...
            db.query(registerQuery, (err, results) => {
                ...
                if (results) {
                  ...
                  db.query(newUserQuery, (err1, results1) => {
                    ...
                    let { idUser, fullName, username, email, idRole, isVerified } =  results1[0];
  ...
  login: (req, res) => {
    //Query 
    let loginQuery = `SELECT * FROM users WHERE email=${db.escape(req.body.email)};`;
    //inject Query
    db.query(loginQuery, (err, results) => {
      ...
      if (results[0]) {
        //user data
        let { idUser, fullName, username, email, password, idRole, isVerified } =results[0];
        //Compare Password
        bcrypt.compare(req.body.password, password, (error, response) => {
            ...
            res.send({message:"Email And Password Doesn't Match", success:false });
          }
       })  
      }else{
  ...
  getDataUser: (req, res) => {
    ...
    db.query(scriptQuery, (err, results) => {
      if (err) {
        console.log(err);
        res.status(500).send({ message: "Failed To Get User Data", error: err });
      } else {
        delete results[0].password;
  ...
  editDataUser: (req, res) => {
    ...
    db.query(scriptQuery, (err, results) => {
      if (err){
        res.status(500).send({message: "Gagal mengambil data di database", success: false, err,});
        return;
      }else{
        res.status(200).send(results);
      }
  ...
  forgotPassword: (req, res) => {
    //Query
    let selectQuery = `SELECT * FROM users WHERE email =${db.escape(req.body.email)} ;`;
    db.query(selectQuery, (err, results) => {
        ...
        if (results) {
          //Create Token
          let { idUser, fullName, username, email, idRole, isVerified } = results[0];
          ...
          nodemailer.sendMail(mail, (errMail, restMail) => {
            if (errMail) {
              console.log(errMail);
              res.status(500).send({ message: "req forgot password failed", success: false, });
            }
            res.status(200).send({ message: "Request Forgot Password Success  ✔", success: true, });
          });
        }
    });
  },
    ...
    bcrypt.hash(newPassword, saltRounds, (error, hash) => {
        ...
        return console.log(error);
          ...
          db.query(updateQuery, (err, results) => {
            if (err) {
              console.log(err);
              res.status(500).send({ message: "update password failed", success: false });
            }
