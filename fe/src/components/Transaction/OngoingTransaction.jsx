...
const OngoingTransaction = () => {
  ...
  const getOngoingTransaction = async () => {
    try {
      dispatch(fetchOngoingTransaction(idUser, paging.currentPage, sort));
  ...
  const renderDataOngoing = () => {
    ...
    return data.map((data) => {
      return <DataOngoingList data={data} />;
    });
  };
  ...
  return (
    <>
      ...
      <header style={{ backgroundColor: "#f7f7f7" }} className="mt-3">
        <div className="row gx-3">
          <div className="col-lg-10 col-6 col-md-3">
            <p className="mt-2">
              <strong>Ongoing Transaction</strong>
            </p>
          </div>
